...
/**
 * @brief Construct a new RobotUnit.
...
{
    // 1) Load SVG
    this->svg_ = std::make_unique<QSvgRenderer>(QString::fromUtf8(kRobotSvgPath));
    ...
    if (this->svg_ && this->svg_->isValid())
    {
        ...
        {
            this->svg_default_size_ = QSizeF(44.0, 44.0);
        }
    }
    else
    {
...
void
RobotUnit::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
